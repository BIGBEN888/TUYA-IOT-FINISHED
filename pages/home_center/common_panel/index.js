...
Page({
  ...
  data: {
    ...
    bgImage: '../../../image/ironOff.png', //..表示返回当前目录的上一级目录
    isSwitchOn: false,
    powerStatics: 0
  },
  ...
  reName:function (arr){
    for(var i in arr){
      for(var j in arr[i]){
        if(arr[i][j] == 'cur_current'){
          arr[i][j] = '电流(ma)' ;
        } 
        else if(arr[i][j] == 'cur_power') {
          arr[i][j] = '功率(W)' ;
        }
        else {
          arr[i][j] = '电压(V)' ;
    ...
    if (status && status.length) {
      status.map((item) => {
        ...
        if (isExit) {
          ...
          //以下转换电流、功率、电压的英文名称为中文
          for(var i in roDpList){
            for(var j in roDpList[i]){
              if(roDpList[i]['name'] == 'cur_current'){
                roDpList[i]['code'] = '电流(ma)' ;
              } 
              else if(roDpList[i]['name'] == 'cur_power') {
                roDpList[i]['code'] = '功率(W)' ;
              }
              else {
                roDpList[i]['code'] = '电压(0.1V)';                            
              }
            }
          }             
        }
